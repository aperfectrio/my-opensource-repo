...
def assign_task(role, project="MediaWiki"):
    match role:
        case "Junior Developer": task = "Fix a typo or formatting error in the codebase"
        case "Active Developer": task = "Review and test an open pull request"
        case "Senior Developer": task = "Fix a reported bug in the software"
        case "Lead Developer": task = "Design and lead a new feature development"
        case _: task = "Read the CONTRIBUTING file on GitHub"
...
while(1):
    ...
    if not name.isalpha():
        print("Invalid, default name has given")
        name = "User"

    experience = input("Enter experience level : ").strip().lower()
    match experience:
        case "beginner": role = "Junior Developer"
        case "intermediate": role = "Active Developer"
        case "advanced": role = "Senior Developer"
        case "expert": role = "Lead Developer"
        case _: role = "Observer"
    ...
    moreInput = input("Add another developer? (yes/no): ").strip().lower()
    if moreInput == "no":
        break
